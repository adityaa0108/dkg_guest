'use client';

import Link from 'next/link';
import { ChevronRight } from 'lucide-react';
import { ProductCard, type Product } from './ProductCard';

const BRIDAL_ENTRY_PRODUCTS: Product[] = [
  {
    id: 301,
    image: 'https://images.unsplash.com/photo-1519741497674-611481863552?w=800&h=1000&fit=crop',
    title: 'Carnival Fun Birthday Decor',
    rating: 4.5,
    price: 2499,
    originalPrice: 9999,
    discount: 24,
  },
  {
    id: 302,
    image: 'https://images.unsplash.com/photo-1522673607200-164d1b6ce486?w=800&h=1000&fit=crop',
    title: 'Wild West Birthday Decor',
    rating: 4.8,
    price: 2499,
    originalPrice: 9999,
    discount: 24,
  },
  {
    id: 303,
    image: 'https://images.unsplash.com/photo-1520854221256-17451cc331bf?w=800&h=1000&fit=crop',
    title: 'Rockstar Music Birthday Decor',
    rating: 4.0,
    price: 2499,
    originalPrice: 9999,
    discount: 24,
  },
  {
    id: 304,
    image: 'https://images.unsplash.com/photo-1591604466107-ec97de577aff?w=800&h=1000&fit=crop',
    title: 'Mad Scientist Birthday Decor',
    rating: 4.0,
    price: 2499,
    originalPrice: 9999,
    discount: 24,
  },
];

export default function BridalEntryDecorationSection() {
  return (
    <section className="py-12 lg:py-16 bg-gray-100">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="bg-white rounded-2xl p-8 lg:p-12 shadow-sm border border-gray-100">
          <div className="text-center mb-10">
            <h2 className="text-2xl lg:text-3xl font-bold bg-gradient-to-r from-pink-500 via-purple-500 to-pink-500 bg-clip-text text-transparent mb-3">
              Bridal Entry Decoration
            </h2>
            <p className="text-gray-500 text-base lg:text-lg max-w-2xl mx-auto">
              we are thrilled to offer a range of exceptional decoration services tailored to elevate your space.
            </p>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {BRIDAL_ENTRY_PRODUCTS.map((product) => (
              <ProductCard key={product.id} product={product} compact />
            ))}
          </div>

          <div className="text-center mt-10">
            <Link
              href="/categories/decorations/bridal/bridal-entry-decoration"
              className="inline-flex items-center gap-2 text-gray-800 font-semibold hover:text-purple-600 transition-colors"
            >
              EXPLORE MORE
              <ChevronRight className="w-5 h-5" />
            </Link>
          </div>
        </div>
      </div>
    </section>
  );
}
